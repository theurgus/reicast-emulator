...
#ifndef pvrrc
#error "Internal compilation error -- 'pvrrc' not defined"
#else
#undef pvrrc
#endif
...
#else
	#define SHADER_DEBUG D3DCOMPILE_OPTIMIZATION_LEVEL3 /*D3DXSHADER_DEBUG|D3DXSHADER_SKIPOPTIMIZATION*/
#endif
/*
...
#define PS_SHADER_COUNT (384*4)
	...
	// ID3DXFont* font;
	// ID3DXConstantTable* font_shader_consts;
...
#define twidle_tex(format)\
						if (tcw.VQ_Comp)\
					{\
						vq_codebook=(u8*)&vram[sa];\
						if (tcw.MipMapped)\
							sa+=MipPoint[tsp.TexU];\
						##format##to8888_VQ(&pbt,(u8*)&vram[sa],w,h);\
					}\
					else\
					{\
						if (tcw.MipMapped)\
							sa+=MipPoint[tsp.TexU]<<3;\
						##format##to8888_TW(&pbt,(u8*)&vram[sa],w,h);\
					}
#define norm_text(format) \
	...
	if (tcw.StrideSel)\
					...
					{sr=w;}\
					format(&pbt,(u8*)&vram[sa],sr,h);

	typedef void DYNACALL texture_handler_FP(PixelBuffer* pb,u8* p_in,u32 Width,u32 Height);
	...
	*/
	...
	struct TextureCacheData
	{
		...
		{
			printf(texFormatName[tcw.PixelFmt]);
			...
			if (tcw.StrideSel)
				printf(" %d[%d]x%d @ 0x%X",(TEXT_CONTROL&31)*32,8<<tsp.TexU,8<<tsp.TexV,tcw.TexAddr<<3);
			else
...
//			verify(lock_block==0);
			...
			u32 sa=(tcw.TexAddr<<3) & VRAM_MASK;

			if (Texture==0)
			{
				if (tcw.MipMapped && (!(settings.rend.nulldc_PaletteMode>=2 && (tcw.PixelFmt==5 || tcw.PixelFmt==6))) )
				{
			...
			case 7:
				...
				if (tcw.ScanOrder)
				{
					...
					//argb1555to8888(&pbt,(u16*)&vram[sa],w,h);
				}
				else
				{
			...
			case 2:
				...
				{
					//verify(tcw.VQ_Comp==0);
					//argb4444to8888(&pbt,(u16*)&vram[sa],w,h);
			...
			case 5:
				...
				pal_rev=pal_rev_16[tcw.PalSelect];
				if (settings.rend.nulldc_PaletteMode<2)
				{
					PAL4to8888_TW(&pbt,(u8*)&vram[sa],w,h);
				}
				else
				{
					PAL4toX444_TW(&pbt,(u8*)&vram[sa],w,h);
				}
			...
			case 6:
				...
				palette_index = (tcw.PalSelect<<4)&(~0xFF);
				pal_rev=pal_rev_256[tcw.PalSelect>>4];
				if (settings.rend.nulldc_PaletteMode<2)
				{
					PAL8to8888_TW(&pbt,(u8*)&vram[sa],w,h);
				}
				else
				{
					PAL8toX444_TW(&pbt,(u8*)&vram[sa],w,h);
				}
				break;
			default:
			...
			sprintf(file,"g:\\textures\\0x%08x_%08x_%s_%d_VQ%d_TW%d_MM%d_.png",tcw.full,tsp.full,texFormatName[tcw.PixelFmt],Lookups
			,tcw.VQ_Comp,tcw.ScanOrder,tcw.MipMapped);
		...
		void LockVram()
		{
			...
			lock_block = libCore_vramlock_Lock(sa,ea,this);
		}
	};
	...
	TextureCacheData* DYNACALL GenText(TSP tsp,TCW tcw,TextureCacheData* tf)
	{
		//generate texture	
		tf->w=8<<tsp.TexU;
		tf->h=8<<tsp.TexV;
	...
	TextureCacheData* DYNACALL GenText(TSP tsp,TCW tcw)
	{
	...
	IDirect3DTexture9* DYNACALL nulldc_GetTexture(TSP tsp,TCW tcw)
	{	
		if (!is_fmt_supported(tsp,tcw)) {
			return 0;
		}

		u32 addr=(tcw.TexAddr<<3) & VRAM_MASK;
		if (addr==rtt_address)
		{
		...
		if (tf)
		{
			...
			if (settings.rend.nulldc_PaletteMode==1)
			{
				if (tcw.PixelFmt==5)
				{				
					if (tf->pal_rev!=pal_rev_16[tcw.PalSelect])
						tf->Update();
				}
				else if (tcw.PixelFmt==6)
				{
					if (tf->pal_rev!=pal_rev_256[tcw.PalSelect>>4])
						tf->Update();
				}
			}
			tf->Lookups++;
	...
	*/
		...
		if (d3d_init_done)
		{
			...
			if ((FB_R_CTRL.fb_enable && !VO_CONTROL.blank_video) || DC_PLATFORM==DC_PLATFORM_ATOMISWAVE)
			{
				if ( *ptest!=0xDEADC0DE)
				{
					...
					u32* read=(u32*)&vram[vramlock_ConvOffset32toOffset64(src)];
					u32* read_line=read;
	...
	struct TA_context
	{
		...
		f32 invW_max;
		List2<Vertex> verts;
		List0<ModTriangle>	modtrig;
		List0<ISP_Modvol>	global_param_mvo;

		List0<PolyParam> global_param_op;
		List0<PolyParam> global_param_pt;
		List0<PolyParam> global_param_tr;
...
/* put any condition you want to sort on here */
	...
	void DYNACALL SetCurrentTARC(u32 addr)
	{
		addr&=0xF00000;
		//return;
	...
	void DYNACALL SetCurrentPVRRC(u32 addr)
	{
		addr&=0xF00000;
		//return;
...
// -> function to use for texture lookup.One of TextureLookup_Normal,TextureLookup_Palette,TextureLookup_Palette_Bilinear
	...
	void CompilePS(u32 mode,const char* profile,FILE* f)
	{
		...
		D3DCompileFromFile(L"ps_hlsl.fx", ps_macros, NULL, "PixelShader_main", profile, SHADER_DEBUG, 0, &shader, &perr);
...
__error_out:
	...
	void DYNACALL ResrotePS()
	{
		dev->SetPixelShader(compiled_ps[last_ps_mode]);
	}
	void DYNACALL SetPS(u32 mode)
	{
	...
	void SetGPState_ps(PolyParam* gp)
	{
		...
		if (gp->pcw.Texture)
		{
			if (settings.rend.nulldc_PaletteMode>1)
			{
				u32 pal_mode=settings.rend.nulldc_PaletteMode-1;
				if (pal_mode==2 && gp->tsp.FilterMode==0)
					pal_mode=1;//no filter .. ugh
					
				u32 pf=gp->tcw.PixelFmt;
				if (pf==5)
				{
					cur_pal_index[1]=gp->tcw.PalSelect/64.0f;
					mode|=pal_mode;
					dev->SetPixelShaderConstantF(0,cur_pal_index,1);
				}
				else if (pf==6)
				{
					cur_pal_index[1]=(gp->tcw.PalSelect&~0xF)/64.0f;
					mode|=pal_mode;
					dev->SetPixelShaderConstantF(0,cur_pal_index,1);
				}
			}
	...
	{	/*
		...
		if (cache_stencil_modvol_on!=stencil && settings.rend.nulldc_ModVolMode==MVM_NormalAndClip)
		{
		...
		if ((gp->tcw.full != cache_tcw.full) || (gp->tsp.full!=cache_tsp.full) || (cache_texture_enabled!= gp->pcw.Texture))
		{
			...
			if ( gp->tsp.FilterMode == 0 || (settings.rend.nulldc_PaletteMode>1 && ( gp->tcw.PixelFmt==5|| gp->tcw.PixelFmt==6) ))
			{
			...
			if (gp->pcw.Texture)
			{
				IDirect3DTexture9* tex= nulldc_GetTexture(gp->tsp,gp->tcw);
				r_set_texture(0,tex);
				float tsz[4];
	...
	template <u32 Type,bool FFunction,bool SortingEnabled>
	void RendPolyParamList(List0<PolyParam>& gpl)
	{
		if (gpl.used==0)
			return;
	...
	void SortRendPolyParamList(List0<PolyParam>& gpl)
	{
...
//			printf("ZMAE: %f %f\n",c0[0],c1[0]);
			...
			*/
			...
			else
			{
				...
				if (settings.rend.nulldc_AspectRatioMode==2 && FB_Y_CLIP.min==0 && FB_Y_CLIP.max==479 && FB_X_CLIP.min==0 && FB_X_CLIP.max==639)
				{
			...
			//stencil modes
			if (settings.rend.nulldc_ModVolMode==MVM_NormalAndClip)
			{
			...
			//OP mod vols
			if (settings.rend.nulldc_ModVolMode !=MVM_Off && pvrrc.modtrig.used>0)
			{
				if(ZBufferCF & D3DCLEAR_STENCIL && (settings.rend.nulldc_ModVolMode==MVM_Normal || settings.rend.nulldc_ModVolMode==MVM_NormalAndClip ))
				{
					/*
					mode :
					normal trig : flip
				...
				else if (settings.rend.nulldc_ModVolMode==MVM_Volume)
				{
					//TODO: Find out what ZPixelShader was supposed to be doing.
					verifyc(dev->SetPixelShader(ZPixelShader));
					r_set_state(R_ALPHABLENDENABLE,TRUE);
			...
			{
				if (dosort && settings.rend.nulldc_AlphaSortMode==1)
					SortPParams();

				if (dosort && settings.rend.nulldc_AlphaSortMode == 2)
				{
					if (UseFixedFunction)
					{
	...
	IDirect3DVertexShader9* CompileVS(const wchar_t* file, const char* function, const char* vsp, const D3D_SHADER_MACRO* pDefines)
	{
		IDirect3DVertexShader9* rv=0;
		ID3DBlob* perr;
		ID3DBlob* shader;
		
		D3DCompileFromFile(file, pDefines, NULL, function, vsp, SHADER_DEBUG, 0, &shader, &perr);
	...
	IDirect3DPixelShader9* CompilePS(const wchar_t* file, const char* function,const D3D_SHADER_MACRO* pDefines)
	{
		IDirect3DPixelShader9* rv=0;
		ID3DBlob* perr;
		ID3DBlob* shader;

		D3DCompileFromFile(file
			,pDefines,NULL,function,"sm_3_0",SHADER_DEBUG, 0,&shader,&perr);
		if (perr)
		{
			char* text=(char*)perr->GetBufferPointer();
			printf("%s\n",text);
	...
	{
		RECT cr;
		GetClientRect((HWND)libPvr_GetRenderTarget(),&cr);
		
		int win_width=cr.right-cr.left;
		int win_height=cr.bottom-cr.top;
		switch(settings.rend.nulldc_ResolutionMode)
		{
		case 0:
		default:
			win_height=-1;
	...
	u32 RenderThead_internal(void* param)
	{
		...
		LoadSettings();
		ZBufferMode=settings.rend.nulldc_ZBufferMode;
		...
		if (UseFixedFunction)
		{
			if (settings.rend.nulldc_PaletteMode>1)
			{
				...
				settings.rend.nulldc_PaletteMode=1;
			}
			if (ZBufferMode==1)
			{
				ZBufferMode=FZB?0:2;
		...
		ppar.PresentationInterval=settings.rend.nulldc_VSync?D3DPRESENT_INTERVAL_ONE:D3DPRESENT_INTERVAL_IMMEDIATE;
		ppar.Windowed =   TRUE;

		ppar.EnableAutoDepthStencil=TRUE;
		ppar.AutoDepthStencilFormat = ZBufferMode==0 ? D3DFMT_D24FS8 : D3DFMT_D24S8;

		ppar.MultiSampleType = (D3DMULTISAMPLE_TYPE)settings.rend.nulldc_MultiSampleCount;
		ppar.MultiSampleQuality = settings.rend.nulldc_MultiSampleQuality;
		...
		if (UseSVP || FAILED(d3d9->CreateDevice(D3DADAPTER_DEFAULT,D3DDEVTYPE_HAL,(HWND)libPvr_GetRenderTarget(),/*D3DCREATE_MULTITHREADED|*/
			D3DCREATE_HARDWARE_VERTEXPROCESSING,&ppar,&dev)))
		{
			...
			UseSVP=true;
			verifyc(d3d9->CreateDevice(D3DADAPTER_DEFAULT,D3DDEVTYPE_HAL,(HWND)libPvr_GetRenderTarget(),/*D3DCREATE_MULTITHREADED|*/
...
//		safe_release(font);
//		safe_release(font_shader_consts);
	...
	f32 vrf(u32 addr)
	{
		return *(f32*)&vram[vramlock_ConvOffset32toOffset64(addr)];
	}
	u32 vri(u32 addr)
	{
		return *(u32*)&vram[vramlock_ConvOffset32toOffset64(addr)];
	}
	...
	void StartRender()
	{
		...
		if (old_pal_mode!=settings.rend.nulldc_PaletteMode)
		{
			//mark pal texures dirty
			TexCacheList<TextureCacheData>::TexCacheEntry* ptext= TexCache.plast;
			while(ptext)
			{
				if ((ptext->data.tcw.PixelFmt == 5) || (ptext->data.tcw.PixelFmt == 6))
				{
			...
			old_pal_mode=settings.rend.nulldc_PaletteMode;
		}
		...
		bg_d.i=ISP_BACKGND_D.i & ~(0xF);
		bg_t.full=ISP_BACKGND_T.full;
	...
	{
		*(u32*)&vram[vramlock_ConvOffset32toOffset64(adr)]=data;
	}
	void EndRender()
	{
		if (!RenderWasStarted)
...
*/
		...
		while(ptext && ((FrameNumber-ptext->data.LastUsed)>60))
		{
			...
			if (settings.rend.nulldc_TexCacheMode==0 || ptext->data.dirty==true)
			{
				ptext->data.Destroy();
				ptext->data.Texture->Release();
				TexCache.Remove(ptext);
		...
		do
		{
			...
			bool same_res=memcmp(&nwr,&resizerq.new_size,sizeof(NDC_WINDOW_RECT))==0 && settings.rend.nulldc_ResolutionMode==old_res_mode;
		...
		{
			//resize
			old_res_mode=settings.rend.nulldc_ResolutionMode;
			d3d_do_resize=true;
			//Restart renderer
			render_restart=false;
			d3d_do_restart=true;
...
#define poly_float_color(to,src) \
	...
	//poly param handling
	__forceinline
		static void DYNACALL AppendPolyParam0(TA_PolyParam0* pp)
		{
			glob_param_bdc;
		}
		__forceinline
		static void DYNACALL AppendPolyParam1(TA_PolyParam1* pp)
		{
			glob_param_bdc;
			poly_float_color(FaceBaseColor,FaceColor);
		}
		__forceinline
		static void DYNACALL AppendPolyParam2A(TA_PolyParam2A* pp)
		{
			glob_param_bdc;
		}
		__forceinline
		static void DYNACALL AppendPolyParam2B(TA_PolyParam2B* pp)
		{
			poly_float_color(FaceBaseColor,FaceColor);
			poly_float_color(FaceOffsColor,FaceOffset);
		}
		__forceinline
		static void DYNACALL AppendPolyParam3(TA_PolyParam3* pp)
		{
			glob_param_bdc;
		}
		__forceinline
		static void DYNACALL AppendPolyParam4A(TA_PolyParam4A* pp)
		{
			glob_param_bdc;
		}
		__forceinline
		static void DYNACALL AppendPolyParam4B(TA_PolyParam4B* pp)
		{
