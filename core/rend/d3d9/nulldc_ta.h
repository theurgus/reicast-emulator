...
namespace TASplitter
{
	...
	void TA_ListInit();
	void TA_SoftReset();
	extern void DYNACALL Dma(u32* data,u32 size);
    extern void DYNACALL SQ(u32* data);
...
#define IsModVolList(list) (((list)&1)!=0)
	...
	typedef u32 DYNACALL TaPolyParamFP(void* ptr);
	...
	public:
		...
		static void DYNACALL ta_list_start(u32 new_list)
		{
		...
		__forceinline
		static Ta_Dma* DYNACALL ta_handle_poly(Ta_Dma* data,Ta_Dma* data_end)
		{
...
#undef ver_64B_def
		...
		static Ta_Dma* DYNACALL ta_dummy_32(Ta_Dma* data,Ta_Dma* data_end)
		{
			TaCmd=ta_main;
			return data+SZ32;
		}
		static Ta_Dma* DYNACALL ta_modvolB_32(Ta_Dma* data,Ta_Dma* data_end)
		{
		...
		static Ta_Dma* DYNACALL ta_mod_vol_data(Ta_Dma* data,Ta_Dma* data_end)
		{
			TA_VertexParam* vp=(TA_VertexParam*)data;
			if (data==data_end)
			{
		...
		static Ta_Dma* DYNACALL ta_spriteB_data(Ta_Dma* data,Ta_Dma* data_end)
		{
			//32B more needed , 32B done :)
			TaCmd=ta_main;
		...
		static Ta_Dma* DYNACALL ta_sprite_data(Ta_Dma* data,Ta_Dma* data_end)
		{
		...
		static Ta_Dma* DYNACALL ta_poly_data(Ta_Dma* data,Ta_Dma* data_end)
		{
...
strip_end:
		...
		static void DYNACALL AppendPolyParam2Full(Ta_Dma* pp)
		{
			TA_decoder::AppendPolyParam2A((TA_PolyParam2A*)&pp[0]);
			TA_decoder::AppendPolyParam2B((TA_PolyParam2B*)&pp[1]);
		}

		static void DYNACALL AppendPolyParam4Full(Ta_Dma* pp)
		{
		...
		template <int t>
		static Ta_Dma* DYNACALL ta_poly_B_32(Ta_Dma* data,Ta_Dma* data_end)
		{
			if (t==2)
...
#define group_EN() if (data->pcw.Group_En){ TA_decoder::TileClipMode(data->pcw.User_Clip);}
		static Ta_Dma* DYNACALL ta_main(Ta_Dma* data,Ta_Dma* data_end)
		{
			do
			{
				...
				case ParamType_End_Of_List:
					{
						...
						//printf("End list %X\n",CurrentList);
						asic_RaiseInterrupt(ListEndInterrupt[CurrentList]);
		...
		*/
		//helpers 0-14
		static u32 DYNACALL poly_data_type_id(PCW pcw)
		{
			if (pcw.Texture)
			{
				//textured
		...
		//0-4 | 0x80
		static u32 DYNACALL poly_header_type_size(PCW pcw)
		{
			if (pcw.Volume == 0)
			{
